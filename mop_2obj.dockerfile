...
USER nonroot
COPY --from=build --chown=nonroot:nonroot --chmod=711 /app/Mazda_CdMOBP/Mazda_CdMOBP/bin/Linux/mazda_mop_sca .
COPY --from=build --chown=nonroot:nonroot /app/wrapper .
COPY ./schema/mazda_mop_sca.json ./schema/
ENV EVAL_MODULE=mazda_mop_sca
CMD ["./wrapper"]